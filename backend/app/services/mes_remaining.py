...
def add_mes_credit(db, out, orders, as_of=None):
    from app.services.remaining_work import required_qty_by_operation
    ...
    if not orders:
        return []
    items = {i.code.upper(): i for i in db.query(Item).options(selectinload(Item.operations), selectinload(Item.bom_lines)).all()}
    ...
    if as_of:
        ship_query = ship_query.filter(Shipment.ship_date <= as_of)
    ...
    for model in (Reservation, Shipment):
        ...
        if model is Shipment and as_of:
            q = q.filter(model.ship_date <= as_of)
    ...
    for order in orders:
        ...
        if order.id in batch_by_anchor and order.quantity:
            factor = batch_by_anchor[order.id].quantity / order.quantity
            required = {op_id: qty * factor for op_id, qty in required.items()}
