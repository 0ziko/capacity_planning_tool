...
interface DowntimeOut {
  totals: { work_center_code: string; expected_minutes: number; actual_minutes: number; machine_minutes?: number; excess_minutes: number }[];
  daily: { work_center_code: string; day: string; expected_minutes: number; actual_minutes: number; machine_minutes?: number; excess_minutes: number }[];
  reasons: { work_center_code: string; reason_code: string; reason_desc: string; minutes: number; measured_labor_minutes?: number; count: number; share_pct: number; proportional_allocation_minutes?: number; excess_attributed_minutes: number }[];
}
interface CtOut {
  rows: { item_code: string; product_group: string; work_center_code: string; operation_seq: number; defined_ct_sec: number; observed_median_ct_sec: number; observed_min_ct_sec: number; observed_max_ct_sec: number; samples: number; deviation_pct: number; measurement_source?: string; suggested_ct_sec: number | null; status: string }[];
...
export default function Analysis() {
  ...
  return (
    <>
      ...
      <h2>Duruşlar — beklenen adam-dakika vs ölçülen <button className="secondary small" onClick={() => api.download(`/api/analysis/downtime.xlsx${qs({ start, end, work_center_ids: wcIds })}`)}>⬇ Excel raporu</button></h2>
      <p className="muted" style={{ marginTop: -8 }}>Gerçekleşen sütunu yalnızca <b>elapsed_minutes</b> / <b>labor_minutes</b> temelli kayıtları toplar; legacy kayıtlar ölçüme dahil edilmez. Makine-dakika ayrı KPI’dır. Sebep tablosundaki &quot;oransal dağıtım&quot; nedensel kayıp değildir.</p>
      <ErrorText err={dt.err} />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 14 }}>
        <div className="table-wrap">
          <table>
            <thead><tr><th>İş Merkezi</th><th className="num">Beklenen adam-dk</th><th className="num">Ölçülen adam-dk</th><th className="num">Makine-dk</th><th className="num">Fazla adam-dk</th></tr></thead>
            <tbody>{dt.data?.totals.map((t) => <tr key={t.work_center_code}><td><b>{t.work_center_code}</b></td><td className="num">{fmt(t.expected_minutes, 0)}</td><td className="num">{fmt(t.actual_minutes, 0)}</td><td className="num">{fmt(t.machine_minutes ?? 0, 0)}</td><td className="num" style={{ color: t.excess_minutes > 0 ? "var(--bad)" : "var(--ok)" }}>{fmt(t.excess_minutes, 0)}</td></tr>)}</tbody>
          </table>
        </div>
        <div className="table-wrap">
          <table>
            <thead><tr><th>İş Merkezi</th><th>Sebep</th><th className="num">Ham süre (dk)</th><th className="num">Ölçülen adam-dk</th><th className="num">Adet</th><th className="num">Pay</th><th className="num">Oransal dağıtım (dk)</th></tr></thead>
            <tbody>{dt.data?.reasons.map((r, i) => <tr key={i}><td>{r.work_center_code}</td><td><b>{r.reason_code}</b> {r.reason_desc}</td><td className="num">{fmt(r.minutes, 0)}</td><td className="num">{fmt(r.measured_labor_minutes ?? 0, 0)}</td><td className="num">{r.count}</td><td className="num">{fmt(r.share_pct)}%</td><td className="num">{fmt(r.proportional_allocation_minutes ?? r.excess_attributed_minutes, 0)}</td></tr>)}</tbody>
          </table>
        </div>
      </div>
      ...
      <div className="panel row">
        ...
        <span className="muted">Otomatik CT önerisi yalnızca fiili süre (reported_hours) ve uygun miktar ile ölçülen kayıtlardan üretilir; kazanılan saat payı dağıtımı kullanılmaz.</span>
      </div>
