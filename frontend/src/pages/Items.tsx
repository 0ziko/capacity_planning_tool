...
export default function Items() {
  ...
  const list = useAsync(() => api.get<Item[]>(`/api/items${qs({ q, limit: 300 })}`), [q]);
    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 14 }}>
        <div className="table-wrap">
          <table>
            <thead><tr><th>Stok Kodu</th><th>Ad</th><th>Grup</th></tr></thead>
            <tbody>
              {list.data?.map((i) => (
                <tr key={i.id} onClick={() => setSel(i)} style={{ cursor: "pointer", background: sel?.id === i.id ? "#e3f2fd" : undefined }}>
                  <td><b>{i.code}</b></td><td>{i.name}</td><td>{i.product_group}</td>
                </tr>
              ))}
            </tbody>
          </table>
