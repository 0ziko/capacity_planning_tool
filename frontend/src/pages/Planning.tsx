...
export default function Planning() {
  ...
  const runAuto = async () => {
    ...
    setBusy(true); setErr("");
    try {
      setResult(await api.post<AutoResult>("/api/plan/auto", { start_week: start, weeks, work_center_ids: wcIds.length ? wcIds : null, replace_existing: true }));
      refresh();
  ...
  return (
    <>
      ...
      <div className="panel row">
        ...
        {can("poweruser") && (
          <>
            <button onClick={runAuto} disabled={busy}>▶ Otomatik planla (termine göre)</button>
          ...
          <tbody>
            {shownLines.map((l) => (
              <tr key={l.id}>
                ...
                <td><span className={`badge ${l.mode === "manual" ? "warn" : "muted"}`}>{l.mode === "manual" ? "manuel" : "oto"}</span></td>
