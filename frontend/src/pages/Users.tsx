...
const ROLE_MATRIX: { v: Role; l: string; d: string }[] = [
  { v: "owner", l: "Owner", d: "Owner panel + admin yetkilerinin tamamı" },
...
export default function Users() {
  ...
  const assignable = ROLE_MATRIX.filter((r) => r.v !== "owner" || can("owner"));
    ...
    <>
      ...
      <div className="panel">
        <table style={{ width: "auto" }}>
          ...
          <tbody>{ROLE_MATRIX.filter((r) => r.v !== "owner" || can("owner")).map((r) => <tr key={r.v}><td><b>{r.l}</b></td><td>{r.d}</td></tr>)}</tbody>
        </table>
      </div>
      <div className="panel row">
        ...
        <label>Rol<select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as Role })}>{assignable.map((r) => <option key={r.v} value={r.v}>{r.l}</option>)}</select></label>
          ...
          <tbody>
            {list.data?.map((u) => (
              <tr key={u.id}>
                ...
                <td>
                  <select value={u.role} disabled={u.role === "owner" && me?.role !== "owner"} onChange={(e) => patch(u, { role: e.target.value })}>
                    {assignable.map((r) => <option key={r.v} value={r.v}>{r.l}</option>)}
                    {u.role === "owner" && me?.role !== "owner" && <option value="owner">Owner</option>}
                  </select>
                </td>
