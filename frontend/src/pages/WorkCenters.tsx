...
const emptyWc = {
  planning_mode: "labor" as "labor" | "line", required_crew_size: 1,
...
const EMPTY_FILTERS: WcFilters = { code: "", area: "", planned: "", active: "" };
...
export default function WorkCenters() {
  ...
  const [detailTab, setDetailTab] = useState<"shifts" | "machines" | "weeks">("weeks");
  ...
  return (
    <>
      ...
      <p className="muted" style={{ marginTop: -6 }}>
        Kapasite için kişi sayısını haftalık iş gücü sekmesinden girin. Satıra tıklayarak haftalık iş gücü, vardiya ve makine detaylarını açın. Yalnızca <b>Planlanıyor</b> işaretli iş merkezleri otomatik planlamaya girer.
      </p>
      <ErrorText err={edit ? cap.err : err || cap.err} />
      ...
      {edit && (
        <div className="panel">
          ...
          <div className="row">
            ...
            <label>Kişi başı verimli saat<input type="number" step="0.25" disabled={edit.planning_mode === "line"} title={edit.planning_mode === "line" ? "Hat modunda kapasite istasyonların haftalık saatlerinden hesaplanır" : ""} value={edit.default_efficient_hours ?? 4} onChange={(e) => setEdit({ ...edit, default_efficient_hours: Number(e.target.value) })} /></label>
            ...
            <label>Hat kapasitesiyle planla<input type="checkbox" checked={edit.planning_mode === "line"} onChange={(e) => setEdit({ ...edit, planning_mode: e.target.checked ? "line" : "labor" })} /></label>
            {edit.planning_mode === "line" && <span className="muted">Ekip ve haftalık saatler istasyon bazında tanımlanır.</span>}
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
              ...
              return (
                <Fragment key={wc.id}>
                  ...
                  >
                    ...
                    <td className="num">{c ? fmt(c.capacity_hours, 0) : "…"} {wc.planning_mode === "line" ? "hat-sa" : "kişi-sa"}</td>
                    ...
                    <td>
                      ...
                      {wc.is_active && wc.is_planned && <span className="badge ok">aktif</span>}
                    </td>
                  </tr>
                  {open && (
                    <tr>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="muted">{wcs.length === 0 ? "İş merkezi yok. «+ Yeni iş merkezi» veya Excel Import kullanın." : "Filtreye uyan kayıt yok."}</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function WcDetail({ wc, cap, week, tab, setTab, canEdit, onEdit, onDelete, onTogglePlanned, onChanged }: {
  wc: WorkCenter; cap?: Capacity; week: string; tab: "shifts" | "machines" | "weeks"; setTab: (t: "shifts" | "machines" | "weeks") => void;
  canEdit: boolean; onEdit: () => void; onDelete: () => void; onTogglePlanned: () => void; onChanged: () => void;
}) {
  return (
    <div style={{ padding: 12 }} onClick={(e) => e.stopPropagation()}>
      <div className="row" style={{ marginBottom: 10, alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <span className="muted">
          Birim: {wc.capacity_unit_hours} sa · {wc.planning_mode === "line" ? "İstasyon bazında hat kapasitesi" : `Varsayılan ${wc.default_efficient_hours} sa/kişi`}
      ...
      <div className="tabs" style={{ margin: "0 0 10px" }}>
        ...
        <button type="button" className={`tab ${tab === "weeks" ? "active" : ""}`} onClick={() => setTab("weeks")}>{wc.planning_mode === "line" ? "Haftalık hat saatleri" : "Haftalık iş gücü"}</button>
      </div>
...
function Machines({ wc, canEdit, onChanged }: { wc: WorkCenter; canEdit: boolean; onChanged: () => void }) {
  ...
  const save = async () => {
    ...
    setErr("");
    try {
      const body = { code: draft.code.trim(), name: draft.name, description: draft.description, is_active: draft.is_active, required_crew_size: draft.required_crew_size ?? null };
  ...
  return (
    <div>
      <div className="table-wrap">
        <table>
          <thead><tr><th>Kod</th><th>Ad</th><th>Açıklama</th>{wc.planning_mode === "line" && <th>Gerekli ekip (kişi)</th>}<th>Aktif</th><th></th></tr></thead>
          <tbody>
            {wc.machines.map((m) => (
              <tr key={m.id} style={{ opacity: m.is_active ? 1 : 0.5 }}>
                ...
                {wc.planning_mode === "line" && <td>{m.required_crew_size ?? "Eksik giriş"}</td>}
            ...
            {wc.machines.length === 0 && <tr><td colSpan={5} className="muted">İstasyon tanımı yok.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {draft && (
        <div className="row" style={{ marginTop: 8 }}>
          ...
          {wc.planning_mode === "line" && <label>Gerekli ekip (kişi)<input type="number" min={1} step={1} value={draft.required_crew_size ?? ""} onChange={e => setDraft({...draft, required_crew_size: e.target.value === "" ? null : Number(e.target.value)})} /></label>}
...
function Shifts({ wc, canEdit, onChanged }: { wc: WorkCenter; canEdit: boolean; onChanged: () => void }) {
  ...
  return (
    <div>
      <div className="table-wrap">
        <table>
          <thead><tr><th>Vardiya</th><th>Günler</th><th>Saat</th><th className="num">Varsayılan verimli sa/kişi</th><th></th></tr></thead>
          <tbody>
            {wc.shifts.map((s) => {
            ...
            {wc.shifts.length === 0 && (
              <tr><td colSpan={5} className="muted">Vardiya yok — varsayılan Pzt–Cum mesai kullanılır.</td></tr>
            )}
          </tbody>
        </table>
      </div>
