...
## Şu Anki Odak
v0.4: kullanıcının gerçek iş merkezi listesi (17 İM, alan gruplu) tanımlandı; makine tanımları + personel→makine ataması + İM bazında kapasite kaynağı seçimi (İM personeli / makine atamaları) eklendi. Kullanıcının sıradaki işi makineleri tanımlamak. Önceki (v0.3): sipariş odaklı planlama görünümleri (bitiş tarihi, İM bazlı, ilerleme, birleştirme) + birim fiyat/ciro raporu + iki planlama modu (termin / maksimum ciro) ve karşılaştırma. Kullanıcı kendi verisini (PRS3 iş merkezi, 6005510 stoku, "Deneme" siparişi — birim fiyat 1850 girildi) girerek test ediyor. Sıradaki adım: kullanıcının yeni geri bildirimleri.
